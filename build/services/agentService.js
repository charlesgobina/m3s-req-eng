...
Respond with ONLY the role name (e.g., "Product Owner", "Business Analyst", etc.)`;
    ...
    async chatWithAgent(message, taskId, subtask, agentRole, sessionId, projectContext) {
        const task = this.learningTasks.find((t) => t.id === taskId);
        ...
        const systemPrompt = this.buildTeamMemberPrompt(member, task, subtask, projectContext, sessionId);
    ...
    async validateSubmission(submission, taskId, subTask, sessionId, projectContext) {
        const task = this.learningTasks.find((t) => t.id === taskId);
        if (!task)
            throw new Error("Invalid task");
        const systemPrompt = this.buildValidationPrompt(task, subTask, projectContext);
    ...
    buildTeamMemberPrompt(member, task, subTask, projectContext, sessionId) {
...
Remember: You're a real professional helping a colleague learn through collaboration, not an AI giving instructions. Be human, be helpful, and guide through discovery.`;
    ...
    buildValidationPrompt(task, subTask, projectContext) {
