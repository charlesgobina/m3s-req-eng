...
export class VectorMemoryService {
        ...
        userData.allConversations.forEach((conversation) => {
            memoryContents.push({
                content: `${conversation.role === "user"
                    ? "Student"
                    : conversation.agentRole || "Assistant"}: ${conversation.content}`,
    ...
    async ragSearch(userId, query) {
        ...
        try {
            ...
            const { data: memories } = await this.supabaseClient.rpc("match_user_memory_embeddings", {
...
To enable full memory features, please set up the user_memory_embeddings table in Supabase.`;
        ...
        try {
            ...
            if (lastStepData && lastStepData.length > 0) {
                ...
                if (lastStepId !== currentStepId) {
                    ...
                    console.log(`   └─ Triggering incremental embedding update for navigation`);
                ...
                console.log(`🔄 [STEP-NAVIGATION] First time navigation for user ${userId} to ${currentStepId}`);
                console.log(`   └─ Processing all completed steps for initial embedding`);
                await this.processUserStepsForEmbedding(userId);
            }
        }
        catch (error) {
    ...
    async forceRefreshEmbeddings(userId) {
        console.log(`🔄 [FORCE-REFRESH] Forcing incremental embedding update for user ${userId}`);
     ...
     */
    async getStepEmbeddingMetadata(userId, stepId) {
        try {
     ...
     */
    async updateStepEmbeddingMetadata(userId, stepId, stepData) {
        console.log("########################################################");
        ...
        const lastActivityAt = stepData.lastActivityAt
            ? stepData.lastActivityAt instanceof Date
                ...
                : new Date(stepData.lastActivityAt._seconds * 1000 +
     ...
     */
    async stepNeedsEmbedding(userId, stepId, stepData) {
        ...
        if (currentChatCount > metadata.chat_message_count) {
            console.log(`   └─ Step ${stepId} has new chat messages (${currentChatCount} vs ${metadata.chat_message_count})`);
            return true;
        }
        return false;
    }
     ...
     */
    async embedSingleStep(userId, stepId, stepData) {
        console.log(`🔮 [STEP-EMBEDDING] Embedding step ${stepId} for user ${userId}`);
        try {
            ...
            console.log(`✅ [STEP-EMBEDDING] Successfully embedded ${contents.length} items for step ${stepId}`);
        }
        catch (error) {
            console.error(`❌ [STEP-EMBEDDING] Error embedding step ${stepId}:`, error);
        }
    }
    /**
     * Get conversations for a specific step
     */
    async getStepConversations(userId, stepId) {
        try {
            ...
            console.error(`❌ Error fetching step conversations for ${stepId}:`, error);
            return [];
        }
    }
    /**
     ...
     */
    async processUserStepsForEmbedding(userId) {
        ...
        try {
            ...
            for (const doc of progressSnapshot.docs) {
                const data = doc.data();
                if (data.subtasks) {
                    for (const [subtaskKey, subtaskData] of Object.entries(data.subtasks)) {
                        if (subtaskData.steps) {
                            for (const [stepKey, stepData] of Object.entries(subtaskData.steps)) {
                                const fullStepId = `${doc.id}_${subtaskKey}_${stepKey}`;
                                if (await this.stepNeedsEmbedding(userId, fullStepId, stepData)) {
            ...
            console.log(`✅ [STEP-PROCESSING] Processed ${stepsToEmbed} steps needing embedding for user ${userId}`);
        }
        catch (error) {
    ...
    async onStepChange(userId, newStepId) {
        console.log(`🔄 [STEP-CHANGE] User ${userId} navigated to step ${newStepId}`);
        ...
        await this.processUserStepsForEmbedding(userId);
        console.log(`✅ [STEP-CHANGE] Completed step-centric embedding update for user ${userId}`);
    }
