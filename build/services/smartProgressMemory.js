...
export class SmartProgressMemory extends ConversationSummaryBufferMemory {
    ...
    sessionId;
    memoryId;
    constructor(userId, taskId, subtaskId, stepId, sessionId, llm, options) {
        super({
            llm,
            maxTokenLimit: 2000,
            returnMessages: true,
    ...
    async getRelevantContextForAgent(agentRole, userQuestion) {
    ...
    async loadAgentSpecificContext(agentRole, userQuestion) {
        try {
    ...
    async loadRelevantConversations(agentRole, userQuestion) {
        try {
            ...
            for (const doc of stepChatsSnapshot.docs) {
                ...
                const conversationText = chatData.messages
                    ...
                    .join(' ')
                    .toLowerCase() || '';
                const agentRelevance = agentKeywords.filter(keyword => conversationText.includes(keyword.toLowerCase())).length;
                const questionRelevance = questionKeywords.filter(keyword => conversationText.includes(keyword.toLowerCase())).length;
    ...
    async saveAgentInsights(agentRole, userQuestion, agentResponse) {
        try {
            ...
            tasksSnapshot.docs.forEach(taskDoc => {
                ...
                if (taskData.isCompleted) {
                    ...
                    if (taskData.subtasks) {
                        Object.entries(taskData.subtasks).forEach(([subtaskKey, subtaskData]) => {
    ...
    async loadAgentInsights(agentRole) {
        try {
            const insightsRef = db.collection('agent_insights')
                .doc(this.userId)
                .collection('agents')
    ...
    async loadCrossAgentInsights(currentAgentRole) {
        try {
            const insightsRef = db.collection('agent_insights')
                .doc(this.userId)
                .collection('agents');
    ...
    async getDetailedConversationHistory(targetTaskIds = [], targetStepIds = [], maxMessages = 20) {
        try {
            ...
            stepChatsSnapshot.docs.forEach(doc => {
                ...
                const shouldInclude = targetTaskIds.length === 0 ||
    ...
    createConversationSummary(stepId, messages, agentRole) {
        const stepDetails = this.parseStepIdDetails(stepId);
        const meaningfulMessages = messages.filter(msg => msg.content && msg.content.length > 20);
    ...
    filterContextByQuestion(context, userQuestion) {
        return context;
    }
    parseStepIdDetails(stepChatId) {
        try {
