...
export class StepMemory extends ConversationSummaryBufferMemory {
  ...
  async persistToRedis(): Promise<void> {
    ...
    try {
      ...
      await redisMemoryService.setConversationMemory(
        this.userId,
        `${this.taskId}_${this.subtaskId}_${this.stepId}`,
        conversationData
      );
  ...
  async loadFromRedis(): Promise<void> {
    ...
    try {
      const conversationData = await redisMemoryService.getConversationMemory(
        this.userId,
        `${this.taskId}_${this.subtaskId}_${this.stepId}`
      );
